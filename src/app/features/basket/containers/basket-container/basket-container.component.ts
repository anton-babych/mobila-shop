...
export class BasketContainerComponent implements OnInit, OnDestroy{
  ...
  ngOnInit() {
    this.subscriptions.push(
      this.basketService.basketIsClicked.pipe(skip(1)).subscribe(e => {
        if (e)
          ...
          this.basketComponent.animateOut()
            .then(x => {
              this.isVisible = e;
            })
        }
    ...
    this.subscriptions.push(this.basketService.items.subscribe(e => {
      this.basketsData = e;
  ...
  private calculateTotalPrice() {
    this.totalPrice = this.basketsData.reduce((sum, item) => sum + item.price*item.count, 0);
  }
